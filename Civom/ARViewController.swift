...
class ARViewController: UIViewController {
    ...
    @IBAction func addButtonDidTouch(_ sender: UIButton) {
        ...
        sheetController.addAction(UIAlertAction(title: "Cube", style: .default, handler: { (action) in
            self.isLoading = true
            self.virtualObjectLoader.loadVirtualObject(.Cube, loadedHandler: { virtualObjectNode in
                self.handleLoad(virtualNode: virtualObjectNode)
            })
        }))
